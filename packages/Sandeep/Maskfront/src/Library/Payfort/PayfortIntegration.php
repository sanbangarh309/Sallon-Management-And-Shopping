<?php
namespace Sandeep\Maskfront\Library\Payfort;
/**
...
class PayfortIntegration
{
    ...
    public function getRedirectionData($paymentMethod) {
        ...
        $postData = array(
            ...
            'return_url'          => url($this->language.'/payment?r=getPaymentPage'),//'https://mask-app.com/wp-content/plugins/activity_managment/includes/classes/payfort-php-sdk/route.php?r=processResponse',//$this->getUrl('route.php?r=processResponse'),
        );
    ...
    {
        $merchantReference = $this->generateMerchantReference();
        $returnUrl = url($this->language.'/payment?r=merchantPageReturn');//'https://mask-app.com/wp-content/plugins/activity_managment/includes/classes/payfort-php-sdk/route.php?r=merchantPageReturn';//$this->getUrl('route.php?r=merchantPageReturn');
        if(isset($_GET['3ds']) && $_GET['3ds'] == 'no') {
            $returnUrl = url($this->language.'/payment?r=merchantPageReturn');//'https://mask-app.com/wp-content/plugins/activity_managment/includes/classes/payfort-php-sdk/route.php?r=merchantPageReturn&3ds=no';//$this->getUrl('route.php?r=merchantPageReturn&3ds=no');
        }
        $iframeParams              = array(
            'merchant_identifier' => $this->merchantIdentifier,
            'access_code'         => $this->accessCode,
    ...
    public function processResponse()
    {
        ...
        if(!$success) {
            ...
            // $return_url = 'https://mask-app.com/demo/wp-content/plugins/activity_managment/includes/classes/payfort-php-sdk/error.php?'.http_build_query($p);//$this->getUrl('error.php?'.http_build_query($p));
            $return_url = $_sallon_link.'?'.http_build_query($p);

        }
        else{
            $params['bid'] = $this->booking_id;
            $return_url = 'https://mask-app.com/thank-you-for-booking/?'.http_build_query($params);//$this->getUrl('success.php?'.http_build_query($params));
        }
    ...
    public function processMerchantPageResponse()
    {
        ...
        if(empty($fortParams)) {
            ...
            if(!$success) {
                ...
                // $return_url = 'https://mask-app.com/demo/wp-content/plugins/activity_managment/includes/classes/payfort-php-sdk/error.php?'.http_build_query($p);
                $return_url = $_sallon_link.'?'.http_build_query($p);
    ...
    public function merchantPageNotifyFort($fortParams)
    {
        ...
        $return_url = url($this->language.'/payment?r=processResponse');//'https://mask-app.com/wp-content/plugins/activity_managment/includes/classes/payfort-php-sdk/route.php?r=processResponse';
        ...
        $postData      = array(
            ...
            'language'            => $this->language,
            'return_url'          => $return_url,//$this->getUrl('route.php?r=processResponse'),
        );
