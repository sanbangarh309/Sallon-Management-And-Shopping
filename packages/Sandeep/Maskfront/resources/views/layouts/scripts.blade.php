...
<script type="text/javascript" src="{{ San_Help::san_Asset('js/bootstrap.min.js') }}"></script>
@if($page == 'home')
	<script src="{{ San_Help::san_Asset('js/juicyslider-min.js') }}"></script>
...
@endif
<script src="{{ San_Help::san_Asset('js/sweetalert.js') }}"></script>
@stack('scripts')
@stack('boot_scripts')
@if($page == 'search')
          ...
          $('#shopping-cart #append_cart_data').on('click','a.action-remove-from-cart',function(){
            $.ajax({
                ...
                success  : function(data) { 
                  ...
                  if (html == '') {
                  	var url = "{{url($locale.'/search?type=products&wr=&pr=')}}";
