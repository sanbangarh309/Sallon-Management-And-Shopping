  ...
  <div class="col-sm-12 segment">
    <div class="row">
      ...
      <div class="col-sm-8 ui padded center aligned inverted segment sc-logo hidden-xs">
        <img class="ui fluid centered image" style="height:65px;" src="{{ San_Help::san_Asset('images/logo.png') }}" alt="cheaplolboost logo">
      </div>
    </div>
  </div>
  <form class="form-horizontal " action="@if(Auth::check()){{url($locale.'/postbook/'.Auth::user()->id.'?tab=cartsummary')}}@endif" method="post" id="cart_form">
    {{csrf_field()}}
    <div class="col-sm-12 pad-0 refreshdiv">
      <table id="product-table" class="ui single line celled unstackable small inverted table">
        ...
        <tbody id="append_cart_data">
          @if((isset($cart_data) && $cart_data->isEmpty()) || !isset($cart_data))
          @php($disable = 1)
          <tr style="text-align: center; font-size: 1.1em;" id="no-items-in-cart">
            <td height="60">
              ...
              <br>
              <a href="{{url($locale.'/search?type=products&wr=&pr=')}}">Click here to browse our accounts</a>
              <div class="ui hidden divider"></div>
            </td>
            <td class="collapsing currency-target left aligned"></td>
            <td></td>
          </tr>
          @elseif(isset($cart_data))
          @php($total = 0)
          ...
          <tr>
            <td height="30">
              <h5 class="item-name">{{$cart->product->name}}</h5>
            </td>
            <td height="30">
              <h4 class="ui header" style="color: white;">SAR {{$cart->total}}</h4>
            </td>
            <td><a href="javascript:void(0);" data-id="{{$cart->id}}" class="ui icon button action-remove-from-cart">&times;</a></td>
          </tr>
          @endforeach
          @endif
        </tbody>
        <tfoot class="full-width">
          <tr>
            ...
            <th class="currency-target" data-entity="yes" data-usd="0" colspan="2">
              <h3 class="ui header total-price" ><b id="cart_total_amnt">SAR @if(isset($total)){{$total}}@else 0 @endif</b>
              </h3>
            </th>
          </tr>
        </tfoot>
    ...
    <div class="col-sm-12 pad-0">
      <div id="cart-action-box" class="ui basic center aligned segment" style="background: none !important;">
        <div id="cart-checkout-box" class="two ui text-center stacking buttons mar-top-30">
          <button id="checkout" class="ui yellow button checkout_san_btn" @if(isset($disable) && $disable ==1) disabled="disabled" @endif style="padding-top: 11px;padding-bottom: 11px;">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content col-sm-12 pad-xs-0">
      <div class="modal-header" align="center">
        <center>
          <h2 class="skyblue-txt bline-btm pad-btm-10">{!!San_Help::sanLang('Login')!!}</h2>
        </center>
      ...
      <div id="div-forms">
        <!-- Begin # Login Form -->
        <form class="custom_signup_form" rel="" method="POST" role="form" action="{{url($locale.'/login')}}" id="login-form" autocomplete="off">
          {{ csrf_field() }}
          <div class="modal-body">
            <div id="msg_block"></div>
            <div id="msg_block_success" class="success-msg"></div>
            <div class="form-group">
              <label class="skyblue-txt"><i class="fa fa-user"></i> {!!San_Help::sanLang('Email')!!}</label>
              <input id="login_username" class="form-control cst-control" type="text" name="email" placeholder="{!!San_Help::sanLang('Email')!!} " required>
            </div>
            <div class="form-group">
              <label class="skyblue-txt"><i class="fa fa-lock"></i> {!!San_Help::sanLang('Password')!!}</label>
              <input id="login_password" class="form-control cst-control" type="password" name="password" placeholder="{!!San_Help::sanLang('Password')!!}" required>
            </div>
            <div class="form-group">
              <div class="checkbox">
                <label class="skyblue-txt">
                  <input type="checkbox"> {!!San_Help::sanLang('Remember me')!!}
                </label>
              </div>
            </div>
            <div class="form-groups">
              <a href="javascript:void(0)" id="login_btnn" class="btn btn-5 img-100"><span>{!!San_Help::sanLang('Login')!!}</span></a>
            </div>
          </div>
          <div class="modal-footer">
            <div class="form-group text-center">
              <a href="javascript:void(0)" class="cd-signin lostform">{!!San_Help::sanLang('Forget')!!} {!!San_Help::sanLang('Password')!!}</a><br>
              <span class="">{!!San_Help::sanLang('New User')!!}</span> <a href="javascript:void(0)" role="button" class="myloginpage1">{!!San_Help::sanLang('Register Now')!!}</a>
            </div>
          </div>
        </form>
      </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h3 class="text-center text-uppercase">{!!San_Help::sanLang('Sign Up')!!}</h3>
      </div>
      <div class="modal-body">
        <div class="container-fluid">
          <div id="signup_msg_block"></div>
          <div id="signup_msg_success" class="success-msg"></div>
          <form class="pro_signup_form" id="pro_signup_form" rel="" autocomplete="off" method="POST" role="form" action="{{route('provider_register')}}">
            {{ csrf_field() }}
            <input type="hidden" name="user_type" value="provider">
            <div class="form-group">
              <div class="input-container">
                <input id="username" type="text" class="form-control input-field" name="name" placeholder="{!!San_Help::sanLang('Username')!!}">
                <i class="fa fa-user icon"></i>
              </div>
            </div>
            <div class="form-group">
              <div class="input-container">
                <input id="manager_name" type="text" class="form-control input-field" name="manager_name" placeholder="{!!San_Help::sanLang('Manager Name')!!}">
                <i class="fa fa-user icon"></i>
              </div>
            </div>
            <div class="form-group">
              <div class="input-container">
                ...
                <input id="contact_number" type="text" class="form-control input-field" name="phone" placeholder="{!!San_Help::sanLang('Contact Number with Country Code')!!}">
                <i class="fa fa-phone icon"></i>
              </div>
            </div>
            <div class="form-group">
              <div class="input-container">
                <input id="s_email" type="text" class="form-control input-field" name="email" placeholder="{!!San_Help::sanLang('email')!!}">
            ...
            <div class="form-group">
              <div class="input-container cst-selectt">
                <select name="_type_of_service" id="_type_of_service" class="form-control input-field">
                  ...
                  ';
                  @foreach (config('maskfront.dropdown_fixed') as $key => $value)
            ...
            <div class="form-group">
              <div class="input-container">
                ...
                <input id="street_address" type="text" class="form-control input-field" name="street_address" onclick="showMap()" placeholder="{!!San_Help::sanLang('fetch your street address')!!}" autocomplete="off" value="">
            ...
            <div class="form-group">
              <div class="input-container">
                <input id="user_password" type="text" class="form-control input-field" name="password" placeholder="{!!San_Help::sanLang('Password')!!}" autocomplete="off">
                <i class="fa fa-key icon"></i>
              </div>
            </div>
            <div class="form-group">
              <div class="input-container">
                <input id="user_conrfpassword" type="text" class="form-control input-field" name="user_conrfpassword" placeholder="{!!San_Help::sanLang('Confirm Password')!!}" autocomplete="off">
                <i class="fa fa-key icon"></i>
              </div>
            </div>
            <div class="form-group">
              <div class="input-container cst-selectt">
                <select name="duration" id="_duration" class="form-control input-field">
                  <option value="">{!!San_Help::sanLang('Duration')!!}</option>
            ...
            <div class="form-group">
              <div class="forgetmenot custom-check" id="custom-check">
                ...
                <label for="terms_condition"><span class="rem-text" id="_terms_label">{!!San_Help::sanLang('Terms and Conditions')!!}</span></label>
              </div>
            </div>
            <div class="form-group text-center gap-btm-35">
              <!-- onclick="submitRegister()" -->
              <a href="javascript:void(0)" id="submit_signup" class="btn yell-btn submt-btn">{!!San_Help::sanLang('Sign Up')!!}</a>
            </div>
          </form>
        </div>
      </div>
...
<div id="customer_register" class="modal fade"  role="dialog" tabindex="-1" aria-labelledby="signupModal" aria-hidden="false">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close_signupModal close" data-dismiss="modal" aria-label="Close"><span class="close-hairline"></span></button>
        <h3 class="text-center text-uppercase memyet">{!!San_Help::sanLang('Not a member yet?')!!}</h3>
      </div>
      <div class="modal-body">
        <div id="reg_msg_block"></div>
        <div id="reg_msg_block_success" class="success-msg"></div>
        <form class="custom_signup_form" rel="" method="POST" role="form" action="{{route('member_register')}}" id="customer_form" autocomplete="off">
          {{ csrf_field() }}
          <div class="form-group">
            <div class="input-container">
              <input id="c_username" type="text" class="form-control input-field" name="name" placeholder="{!!San_Help::sanLang('Full Name')!!}">
              <i class="fa fa-user icon"></i>
            </div>
          </div>
          <div class="form-group">
            <div class="input-container">
              <input id="c_user_email" type="text" class="form-control input-field" name="email" placeholder="{!!San_Help::sanLang('Email')!!}">
              <i class="fa fa-envelope icon"></i>
            </div>
          </div>
          <div class="form-group">
            <div class="input-container">
              ...
              <input id="c_contact_number" type="text" class="form-control input-field" name="phone" placeholder="{!!San_Help::sanLang('Contact Number with Country Code')!!}">
              <i class="fa fa-phone icon"></i>
            </div>
          </div>
          ...
          <div class="form-group">
            <div class="input-container cst-selectt">
              <select name="gender" name="gender" id="gender_field">
                <option value="Male">{!!San_Help::sanLang('Male')!!}</option>
                <option value="Female">{!!San_Help::sanLang('Female')!!}</option>
              </select>
            </div>
          </div>
          <div class="form-group">
            <div class="input-container">
              <input id="c_user_password" type="text" class="form-control input-field" name="password" placeholder="{!!San_Help::sanLang('Password')!!}">
              <i class="fa fa-key icon"></i>
            </div>
          </div>
          <div class="form-group">
            <div class="input-container">
              <input id="c_user_conrfpassword" type="text" class="form-control input-field" name="user_conrfpassword" placeholder="{!!San_Help::sanLang('Confirm Password')!!}">
              <i class="fa fa-key icon"></i>
            </div>
          </div>
          <div class="form-group">
            <div class="forgetmenot custom-check" id="custom-check_cust">
              ...
              <label for="c_terms_condition" id="c_terms_label"><span class="rem-text">{!!San_Help::sanLang('Terms and Conditions')!!}</span></label>
            </div>
          </div>
          <div class="form-group text-center">
            <!-- onclick="submitCustRegister()" -->
            <a href="javascript:void(0)" id="submit_customer_signup" class="btn yell-btn submt-btn">{!!San_Help::sanLang('Sign Up')!!}</a>
          </div>
          <div class="modal-footer">
            <div class="col-sm-12 text-center">
              <p>{!!San_Help::sanLang('Are you signed UP?')!!} <a class="reg-link" id="_login_show" href="javascript:void(0)">{!!San_Help::sanLang('Login')!!}</a></p>
            </div>
          </div>
        </form>
      </div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content col-sm-12 pad-xs-0">
      ...
      <div id="div-forms">
        ...
        <form id="lost-form" action="{{url($locale.'/user_forgot_pass')}}" method="post">
          {{ csrf_field() }}
          <div class="modal-body">
            <div class="container-fluid">
              <div class="form-group">
                <input id="lost_email" class="form-control cst-control" name="email" type="text" placeholder="{!!San_Help::sanLang('Email')!!}" required>
              </div>
              <div class="form-group">
                <button type="submit" class="btn btn-5">{!!San_Help::sanLang('Send')!!}</button>
              </div>
            </div>
          </div>
          <div class="modal-footer">
            <div class="last-btngroup text-center">
              <a class="cd-signin myloginpage" href="javascript:void(0)">{!!San_Help::sanLang('Log In')!!}</a>
              <a href="javascript:void(0)" role="button" class="myloginpage1">{!!San_Help::sanLang('Register')!!}</a>
            </div>
          </div>
        </form>
...
@if($page =='user_account' || $page == 'dashboard')
  ...
  <div class="modal-dialog modal-md">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h3 class="text-center modl-head col-sm-12">@if($page == 'dashboard') Reply @else{!!San_Help::sanLang('Leave Feedback')!!}@endif</h3>
      </div>
      <div class="modal-body">
        <div class="container-fluid">
          <form method="post" action="#" id="new-review-formm" class="new-review-form" onsubmit="submitReview(this);return false;">
            <input name="rating" id="rating" type="hidden" value="@if(isset($user->reviews->rating)){{$user->reviews->rating}}@endif">
            ...
            <div class="form-group">
              ...
              <div class="spr-form-input spr-starrating " id="product_star_rating">
                <ul>
                  @for ($i = 1; $i <= 5; $i ++)
                  @php($selected = "")
                  @if (isset($user->reviews->rating) && $i <= $user->reviews->rating)
            ...
            <div class="form-group">
              ...
              <div class="spr-form-input">
                <textarea class="spr-form-input spr-form-input-textarea " id="review_body" name="review" rows="5" placeholder="{!!San_Help::sanLang('Write your comments here')!!}">@if(isset($user->reviews->review)){!!$user->reviews->review!!}@endif</textarea>
              </div>
            </div>
            <div class="form-group">
...
<div id="update_banner_Modal" class="modal fade" role="dialog" aria-hidden="false">
  <div class="modal-dialog modal-md">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close_addgallary close" data-dismiss="modal" aria-label="Close"><span class="close-hairline"></span></button>
        <h3 class="text-center modl-head col-sm-12">{!!San_Help::sanLang('Update Banner Image')!!}</h3>
      </div>
      <div class="modal-body">
        <div class="container-fluid">
          <form class="form-horizontal custom_image_form" enctype="multipart/form-data" id="update_banner_form" rel="" method="POST" role="form" action="{{route('upload_image')}}">
            {{ csrf_field() }}
            <div class="form-group">
            ...
            <div class="form-group text-center">
              <button type="submit" class="btn yell-btn submt-btn" value="update_banner_img" name="update_banner_img">{!!San_Help::sanLang('Submit')!!}</button>
            </div>
          </form>
        </div>
      </div>
...
<div id="update_image_Modal" class="modal fade" role="dialog" aria-hidden="false">
  <div class="modal-dialog modal-md">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close_addgallary close" data-dismiss="modal" aria-label="Close"><span class="close-hairline"></span></button>
        <h3 class="text-center modl-head col-sm-12">{!!San_Help::sanLang('Update Featured Image')!!}</h3>
      </div>
      <div class="modal-body">
        <div class="container-fluid">
          <form class="form-horizontal custom_image_form" enctype="multipart/form-data" id="update_image_form" rel="" method="POST" role="form" action="{{route('upload_image')}}">
            {{ csrf_field() }}
            <div class="form-group">
            ...
            <div class="form-group text-center">
              <button type="submit" class="btn yell-btn submt-btn" value="add_fet_img" name="add_fet_img">{!!San_Help::sanLang('Submit')!!}</button>
            </div>
          </form>
        </div>
      </div>
...
<div id="update_gallary_images" class="modal fade" role="dialog" aria-hidden="false">
  <div class="modal-dialog modal-md">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close_addgallary close" data-dismiss="modal" aria-label="Close"><span class="close-hairline"></span></button>
        <h3 class="text-center modl-head col-sm-12">{!!San_Help::sanLang('Update Gallary Images')!!}</h3>
      </div>
      <div class="modal-body">
        <div class="container-fluid">
...
<div id="update_des_Modal" class="modal fade" role="dialog"  aria-hidden="false">
  <div class="modal-dialog modal-md">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close_addgallary close" data-dismiss="modal" aria-label="Close"><span class="close-hairline"></span></button>
        <h3 class="text-center modl-head col-sm-12">{!!San_Help::sanLang('Wallet Balance')!!}</h3>
      </div>
      <div class="modal-body">
        <div class="container-fluid">
          <form class="form-horizontal form-gutter custom_signup_form" enctype="multipart/form-data" id="custom_signup_form" rel="" method="POST" role="form" action="">
            ...
            <div class="form-group">
              <input name="name" id="name" value="@if(isset($provider->wallet)){{$provider->wallet}}@endif" placeholder="Enter Amount" class="form-control" type="text">
            </div>
            <div class="form-group text-center">
              <button type="submit" class="btn yell-btn submt-btn" value="update_description" name="update_description">{!!San_Help::sanLang('Withdraw')!!}</button>
            </div>
          </form>
        </div>
      </div>
...
<div id="addteam_Modal" class="modal fade" role="dialog" aria-hidden="false">
  <div class="modal-dialog modal-md">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close_addgallary close" data-dismiss="modal" aria-label="Close"><span class="close-hairline"></span></button>
        <h3 class="text-center modl-head text-uppercase col-sm-12" id="add_team_head">{!!San_Help::sanLang('Add Team Member')!!}</h3>
      </div>
      <div class="modal-body">
        <div class="container-fluid">
          <form class="addteam_form" id="addteam_form" rel="" autocomplete="off" method="POST" enctype="multipart/form-data" role="form" action="{{route('addteam')}}">
            {{ csrf_field() }}
            <div class="form-group">
...
<div id="see_user_review_{{$booking->id}}" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="classInfo" aria-hidden="true">
      ...
      <div class="modal-body" style="overflow-x: auto;">
        <table id="classTable" class="table table-bordered">
          ...
          <tbody>
            ...
            <tr>
              <td>{{$user->name}}</td>
              <td>{{$user->user_reviews[0]->rating}}</td>
              <td>{{$user->user_reviews[0]->review}}</td>
              <td>{!! Carbon\Carbon::parse($user->user_reviews[0]->created_at)->format('d F Y') !!}</td>
            </tr>
            @else
            <tr>
...
<div id="revchart_Modal" class="modal fade" role="dialog" aria-hidden="false">
  <div class="modal-dialog modal-md">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close_addgallary close" data-dismiss="modal" aria-label="Close"><span class="close-hairline"></span></button>
        <h3 class="text-center modl-head col-sm-12">{!!San_Help::sanLang('Monthly Revenue Chart')!!}</h3>
      </div>
      <div class="modal-body">
        <div class="container-fluid">
          <form class="form-horizontal custom_revform" enctype="multipart/form-data" id="update_image_form" rel="" method="POST" role="form" action="">
            <div class="well rev-well">
              ...
              <div id="graph-wrapper" class="col-sm-12 pad-0 text-center graph-sector">
                <img src="{{ San_Help::san_Asset('images/rev-chart.png') }}" class="img-responsive" alt="">
              </div>
            </div>
          </form>
        </div>
...
<div id="add_service_Modal" class="modal fade" role="dialog"  aria-hidden="false">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="container-fluid">
          <form class="form-gutter custom_signup_form" enctype="multipart/form-data" id="servicee_form_add" rel="" method="POST" role="form" action="{{route('add_service')}}">
            {{ csrf_field() }}
            <input type="hidden" id="service_url" value="{{ url('admin/get_services') }}">
            <div class="form-group">
              <div class="input-container cst-selectt">
                ...
                <select class="form-control selectpicker san_change_category" name="category_id" required="" data-show-subtext="true" data-live-search="true" onchange="show_services(this.value)" id="san_change_category">
                  ...
                  @foreach(\TCG\Voyager\Models\Category::whereNull('parent_id')->where('type','category')->get() as $category)
            ...
            <div class="form-group">
              <div class="input-container">
                <select class="form-control selectpicker" required="" data-show-subtext="true" data-live-search="true" id="changed_services" name="parent_service">
                  ...
                  @foreach(\TCG\Voyager\Models\Category::whereNotNull('parent_id')->where('type','category')->get() as $category)
                  <option value="{{$category->id}}">{{$category->name}}</option>
                  @endforeach
                </select>
              </div>
...
<div id="add-newproduct" class="modal fade" role="dialog" aria-hidden="false">
  <div class="modal-dialog modal-md" style="max-width:700px;">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close_addgallary close" data-dismiss="modal" aria-label="Close"><span class="close-hairline"></span></button>
        <h3 class="text-center modl-head col-sm-12" id="add_productss">{!!San_Help::sanLang('Add New Product')!!}</h3>
        <h3 class="text-center modl-head col-sm-12" id="edit_productss" style="display: none;">{!!San_Help::sanLang('Edit Product')!!}</h3>
      </div>
      <div class="modal-body">
        <div class="container-fluid add-form">
          <form class="form-horizontal custom_revform" enctype="multipart/form-data" id="product_form_edit_add" rel="" method="POST" role="form" action="{{route('add_product')}}">
            {{ csrf_field() }}
            <div class="well rev-well">
              <div class="form-group">
              ...
              <div class="form-group">
                <div class="input-container cst-selectt">
                  <select name="category" id="product_category" class="form-control input-field selectpicker">
                    ...
                    @foreach(\TCG\Voyager\Models\Category::whereNull('parent_id')->where('type','procategory')->get() as $category)
                    <option value="{{$category->id}}">{{$category->name}}</option>
                    @endforeach
                  </select>
                </div>
              ...
              <div class="form-group">
                <div class="input-container">
                  <section class="cstm-upload">
                    <label for="file" class="input input-file">
                      <div class="button"><input id="service_image" type="file" class="form-control input-field" name="image" onchange="this.parentNode.nextSibling.value = this.value">{!!San_Help::sanLang('Browse')!!}</div>
                      <input placeholder="include some files" class="input-field" readonly="" type="text">
                    </label>
                  </section>
                </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form class="form-horizontal custom_forgot_pass_form" rel="" autocomplete="off" method="POST" role="form" action="">
          <input type="hidden" id = "forget_user_id" value="{{$_GET['uid']}}">
          <div class="form-group">
