...
class hotelsback extends MX_Controller {
		...
		function index() {
                ...
                if ($this->session->pt_accountType =='Admin') {
                	$xcrud->where('hotel_admin',$this->session->pt_logged_id);
                }
				...
				/**** Get Owner*****/
				// $xcrud->join('hotel_admin', 'pt_accounts', 'accounts_id');
				$xcrud->order_by('pt_hotels.hotel_id', 'desc');
				$xcrud->subselect('Owned By', 'SELECT CONCAT(ai_first_name, " ", ai_last_name) FROM pt_accounts WHERE accounts_id = {hotel_admin}');
				/*****************/
				// $xcrud->label('hotel_title', 'Name')->label('hotel_stars', 'Stars')->label('hotel_is_featured', '')->label('hotel_order', 'Order');
				$xcrud->label('hotel_title', 'Name')->label('hotel_stars', 'Stars')->label('hotel_is_featured', '');
				...
				$xcrud->columns('hotel_is_featured,thumbnail_image,hotel_title,hotel_stars,Owned By,hotel_city,hotel_slug,hotel_status');

				$xcrud->search_columns('hotel_is_featured,hotel_title,hotel_stars,Owned By,hotel_city,hotel_status');
...
// Add Hotels
		...
		public function add() {
				...
				if (!empty ($addhotel)) {
						...
						if ($this->form_validation->run() == FALSE) {
								...
								/* @san Section Add By Sandeep Bangarh */	
								$this->load->model('Tours/Guest_types');
					            $this->Guest_types->sanUpdateHotelAgeRange($hotelid,'hotel');
					            /* End */
					    ...
					    $this->data['users'] = $this->Accounts_model->get_active_admins();
		...
		function cat_index($slug='') {
				if ($slug) {
					$this->manage_cat($slug);
				}else{
					if(!$this->data['addpermission'] && !$this->editpermission && !$this->deletepermission){
	                	backError_404($this->data);
	                }else{
						...
						$xcrud->table('san_category');

		                if($this->role == "supplier"){
		                $xcrud->where('cat_owned_by',$this->data['userloggedin']);

		                }
						$xcrud->join('cat_owned_by', 'pt_accounts', 'accounts_id');
						//$xcrud->join('cat_city', 'pt_locations', 'id');
						$xcrud->order_by('san_category.cat_id', 'desc');
						$xcrud->subselect('Owned By', 'SELECT CONCAT(ai_first_name, " ", ai_last_name) FROM pt_accounts WHERE accounts_id = {cat_owned_by}');
						// $xcrud->label('cat_title', 'Name')->label('cat_stars', 'Stars')->label('cat_is_featured', '')->label('cat_order', 'Order');
						$xcrud->label('cat_title', 'Name');
		                if($this->editpermission){
		                $xcrud->button(base_url() . $this->data['adminsegment'] . '/hotels/category/{cat_slug}', 'Edit', 'fa fa-edit', 'btn btn-warning', array('target' => '_self'));
		                $xcrud->column_pattern('cat_title', '<a href="' . base_url() . $this->data['adminsegment'] . '/hotels/category/{cat_slug}' . '">{value}</a>');
		                }

		                if($this->deletepermission){
		                $delurl = base_url().'admin/categoryajaxcalls/delcategory';
		                $xcrud->multiDelUrl = base_url().'admin/categoryajaxcalls/delMultiplehotels';
		                $xcrud->button("javascript: delfunc('{cat_id}','$delurl')",'DELETE','fa fa-times', 'btn-danger',array('target'=>'_self','id' => '{cat_id}'));
		                }
		                $xcrud->limit(50);
						$xcrud->unset_add();
						$xcrud->unset_edit();
						$xcrud->unset_remove();
						$xcrud->unset_view();
						// $xcrud->column_width('cat_order', '7%');
						$xcrud->columns('cat_title,Owned By,cat_slug,cat_status');

						$xcrud->search_columns('cat_title,Owned By,cat_status');
						$xcrud->label('cat_slug', 'Gallery');
						$xcrud->label('cat_status', 'Status');
						// $xcrud->column_callback('san_category.cat_order', 'orderInputcategorys');
						$xcrud->column_callback('cat_slug', 'categoryGallery');
						$xcrud->column_callback('cat_status', 'create_status_icon');
					
						$this->data['content'] = $xcrud->render();
						$this->data['page_title'] = 'Category Management';
						$this->data['main_content'] = 'temp_view';
						$this->data['header_title'] = 'Category Management';
						$this->data['add_link'] = base_url(). $this->data['adminsegment'] . '/hotels/cat_add';
						$this->load->view('Admin/template', $this->data);
					}
				}
		}
		public function cat_add() { 
                ...
                $this->load->model('Admin/Uploads_model');
				$addcat = $this->input->post('submittype');

                $this->data['submittype'] = "cat_add";

				if (!empty ($addcat)) {
						$this->form_validation->set_rules('catname', 'category Name', 'trim|required');
						$this->form_validation->set_rules('catdesc', 'Description', 'trim|required');
						if ($this->form_validation->run() == FALSE) {
								echo '<div class="alert alert-danger">' . validation_errors() . '</div><br>';
						}
						else {
								$categoryid = $this->Hotels_model->add_category($this->data['userloggedin']);
								$this->Hotels_model->add_translation($this->input->post('translated'), $categoryid);
								$this->session->set_flashdata('flashmsgs', 'Category added Successfully');
								echo "done";
						}
				}
				else {
						$this->data['main_content'] = 'Hotels/manage_cat';
						$this->data['page_title'] = 'Add Category';
						$this->data['headingText'] = 'Add Category';
						$this->data['all_hotels'] = $this->Hotels_model->select_related_hotels($this->data['userloggedin']);
		...
		function manage_cat($catlug) {
				if (empty ($catlug)) {
						redirect($this->data['adminsegment'] . '/hotels/manage_cat/');
				}
               if(!$this->editpermission){
                 echo "<center><h1>Access Denied</h1></center>";
                 backError_404($this->data);
				  }else{
				$updatecategory = $this->input->post('submittype');
				...
				if (!empty ($updatecategory)) {
						...
						if ($this->form_validation->run() == FALSE) {
								...
								$this->Hotels_model->update_Category($categoryid);
								$this->session->set_flashdata('flashmsgs', 'Category Updated Successfully');
								echo "done";
						}
				}
				else {
						@ $this->data['hdata'] = $this->Hotels_model->get_cat_data($catlug);
						$this->data['main_content'] = 'Hotels/manage_cat';
						$this->data['page_title'] = 'Manage category';
						$this->data['headingText'] = 'Update ' . $this->data['hdata'][0]->cat_title;
						$this->data['all_hotels'] = $this->Hotels_model->select_related_hotels($this->data['userloggedin']);
						$this->data['catid'] = $this->data['hdata'][0]->cat_id;
						$this->load->view('Admin/template', $this->data);
				}
			}
		}
               ...
               if(!$this->editpermission){
				...
				if (!empty ($updatehotel)) {
					...
					$this->data['users'] = $this->Accounts_model->get_active_admins();
						...
						/* @san Section Add By Sandeep Bangarh */
						$this->load->model('Tours/Guest_types');
                        $this->data['age_range'] = $this->Guest_types->sanGetAgeRange($this->data['hdata'][0]->hotel_id,'hotel');
                         /* End */
...
if(!$this->data['addpermission'] && !$this->editpermission && !$this->deletepermission){
								...
								if ($this->form_validation->run() == FALSE) {
										...
										if (isset ($_FILES['defaultphoto']) && !empty ($_FILES['defaultphoto']['name'])) {
												...
												$roomid = $this->Rooms_model->add_room();
												 /* @san Section Add By Sandeep Bangarh */	
												$this->load->model('Tours/Guest_types');
									            $this->Guest_types->sanUpdateHotelRoomAgeRange($this->input->post('hotelid'),$roomid);
									            /* End */
				...
				elseif ($args == "manage" && !empty ($editroom)) {
						...
						if (!empty ($updateroom)) {
								...
								if ($this->form_validation->run() == FALSE) {
										...
										/* @san Section Add By Sandeep Bangarh */
											$this->load->model('Tours/Guest_types');
									        $this->Guest_types->sanUpdateHotelRoomAgeRange($this->input->post('hotelid'),$room_id);
									    /* End */
								...
								/* @san Section Add By Sandeep Bangarh */
								$this->load->model('Tours/Guest_types');
                        		$this->data['room_ranges'] = $this->Guest_types->sanGetHotelRoomRange($this->data['rdata'][0]->room_hotel,$this->data['rdata'][0]->room_id);
                         		/* End */
		...
		$output .= '<tr><td colspan="39"><div class="dc_part" style="width:16px;height:15px;float:left;margin:1px;"></div> &nbsp;- Maintenance </td></tr>';
		...
		$output .= '<input type="hidden" name="ids_list" value="'.$ids_list.'">';
					   ...
					   //	 $datefmt = $this->input->post('dateformat');
						...
						$this->load->model('Tours/Guest_types');
					    $this->Guest_types->sanAddRoomPrice($priceid,$roomid);
					    /* End */
                        redirect($this->data['adminsegment'].'/hotels/rooms/prices/'.$roomid);
                          ...
                          /* @san Section Add By Sandeep Bangarh */
                          $this->load->model('Tours/Guest_types');
					      $this->Guest_types->sanupdateRoomPrices($this->input->post('pricesdata'));
					      /* End */
                         ...
                         /* @san Section Add By Sandeep Bangarh */
						// $this->load->model('Tours/Guest_types');
      //                   $this->data['updated_price'] = $this->Guest_types->sanGetHotelRoomPrice(32);
                         /* End */
                        ...
                        /* @san Section Add By Sandeep Bangarh */
						$this->load->model('Tours/Guest_types');
                        $this->data['age_range'] = $this->Guest_types->sanGetHotelRoomRange($this->data['room'][0]->hotel_id,$editroom);
                         /* End */
		                ...
		                if ($this->session->pt_accountType =='Admin') {
							$sql = 'SELECT hotel_id FROM pt_hotels WHERE hotel_admin = '.$this->session->pt_logged_id.' AND hotel_status = "Yes"';
							$hotelll = $this->db->query($sql);
							$hdataa = $hotelll->result();
							$xcrud->where('room_hotel',$hdataa[0]->hotel_id);
		                }
